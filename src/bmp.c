...
int bmp_load(char *bmp_fname)
{
	...
	int y,x;
	for (y=1;y<(img->img_header.h+1);y++)
	{
...
//		img->file_header.fsize+=4-(img->file_header.fsize&0x03);
	...
	img->img_header.w=w;
	img->img_header.h=h;
	img->img_header.hres=2835;
	img->img_header.vres=2835;
...
void bmp_set_pixel(int x, int y, uint32_t pixel, int index)
{
	uint32_t *location;
	int bytes_per_pixel = bmp[index]->img_header.bpp>>3;

	location = bmp[index]->pixel + ((((bmp[index]->img_header.h-y) * bmp[index]->img_header.w) + x) << 2);//bytes_per_pixel;

	if (bytes_per_pixel==2)
			*location = (pixel&0x7c00)<<6|(pixel&0x03e0)<<3|(pixel&0x001f); //not tested
	else if (bytes_per_pixel==3)
			*location = (pixel)|0xff000000; //I just hard code 0xff for the destination alpha
	else if (bytes_per_pixel==4)
			*location = pixel;
	return;
}
